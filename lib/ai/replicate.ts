...
/** Modèle TTS par défaut : MiniMax Speech-02 HD (voix naturelles/expressives). */
const DEFAULT_TTS_MODEL = "minimax/speech-02-hd";
...
/**
 * Synthèse vocale via Replicate (MiniMax Speech-02 HD) — utilise le même
 * REPLICATE_API_TOKEN que les images/vidéos (pas de GroupId requis).
 * Retourne l'URL d'un fichier audio, ou { simulated:true } sans token.
 */
export async function generateSpeech(
  text: string,
  voiceId: string,
  opts?: { languageBoost?: string; emotion?: string }
  ...
  const input: Record<string, unknown> = {
    ...
    bitrate: 128000,
    channel: "mono",
    language_boost: opts?.languageBoost ?? "Automatic",
  };
  if (opts?.emotion) input.emotion = opts.emotion;

  const prediction = await runPrediction(DEFAULT_TTS_MODEL, input);
  ...
  const url =
    typeof out === "string"
      ...
      : Array.isArray(out)
      ? String(out[0])
      : (out as { audio?: string } | undefined)?.audio;
  return { url };
}
